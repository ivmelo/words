...
export default class SimpleModel {
    ...
    async save() {
        ...
        let query = 'insert into ' + this.constructor.tableName() + ' (' + properties.join(', ') + ') values (' + placeholders.join(', ') + ')' ;
 ...
 */
SimpleModel.get = async function (id) {
    let db = SQLite.openDatabase(this.databaseName() + '.sqlite');
 ...
 */
SimpleModel.where = async function (filter_hash, operation = 'AND', sort_field = 'id', sort_order = 'ASC', refresh_models = false) {
    try {
 ...
 */
SimpleModel.destroyAll = async function () {
    let db = SQLite.openDatabase(this.databaseName() + '.sqlite');
